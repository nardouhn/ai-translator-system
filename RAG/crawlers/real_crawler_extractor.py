    ...
    for i in range(0, len(vi_titles), batch_size):
        ...
        if r:
            ...
            for pid, pdata in pages.items():
                ...
                if ll:
                    en_term = ll[0].get("*").lower()
                    vi_term = pdata.get("title")
                    
                    # 🛑 Lọc: Không lấy thuật ngữ chứa số
                    if any(char.isdigit() for char in en_term) or any(char.isdigit() for char in vi_term):
                        continue
                        
                    results.append({"english": en_term, "vietnamese": vi_term, "domain": domain})
        time.sleep(0.5)
    return results
