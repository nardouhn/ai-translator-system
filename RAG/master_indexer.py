...
def clean_text(text: str) -> str:
    """Tiền xử lý chuỗi: Xóa rác, đưa về chữ thường, chuẩn hóa khoảng trắng."""
    if not text:
        return ""
    text = text.lower() # Chuyển thành chữ thường
    text = re.sub(r'<[^>]+>', '', text) # Xóa HTML tags
    text = re.sub(r'\s+', ' ', text) # Gộp khoảng trắng thừa
    text = text.strip(" -.,_:;'\"[](){}") # Xóa ký tự vô nghĩa ở đầu/cuối
    return text.strip()

# --- Configuration ---
DB_PATH = "./VectorDB"
BATCH_SIZE = 2000
# Lưu checkpoint ngay tại thư mục hiện tại để tránh lỗi thiếu folder
CHECKPOINT_FILE = "./index_checkpoint.json"

# Mapping of domain folders to their primary files
# Đảm bảo các file này tồn tại trong thư mục package_enterprise/data/...
DOMAINS = {
    "medical_glossary": "data/medical/medical_glossary.json",
    "economic_glossary": "data/economic/economic_glossary.json",
    "economic_glossary_wiki": "data/economic/economic_glossary_wiki_new.json",
...
def run_indexing():
    ...
    for domain_key, file_path in DOMAINS.items():
        ...
        for idx, item in enumerate(items):
            en_raw = item["english"]
            vi_raw = item["vietnamese"]
            
            # --- Tiền xử lý dữ liệu ---
            en_clean = clean_text(en_raw)
            vi_clean = clean_text(vi_raw)
            
            # Bỏ qua nếu sau khi dọn dẹp biến thành chuỗi rỗng
            if not en_clean or not vi_clean:
                continue

            term_id = hashlib.md5(en_clean.encode()).hexdigest()
            full_id = f"glos_{domain_name}_{term_id}"

            # Lọc trùng ngay tại đây để tránh lỗi DuplicateIDError của ChromaDB
            if full_id in seen_ids_in_domain:
                continue
            
            seen_ids_in_domain.add(full_id)
            
            # Đưa chuỗi đã xử lý SẠCH SẼ vào VectorDB
            documents.append(en_clean)
            metadatas.append({
                "english": en_clean,
                "vietnamese": vi_clean,
                "domain": domain_name,
